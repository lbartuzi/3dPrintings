...
//  - Row 3 (bottom): one full-width tray + etched ruler below it
...
board_width        = 210;    // left-right
board_depth        = 210;    // front-back
board_thickness    = 13;     // complete height
...
/* [Row 1 - top row] */
row1_count        = 4;       // number of pockets
row1_last_double  = false;    // true = last pocket spans 2 units (like "K")
row1_height       = 49;      // pocket size front-to-back
row1_pocket_depth = 10;

/* [Row 2 - middle row] */
row2_count        = 4;
row2_height       = 49;
row2_pocket_depth = 10;

/* [Row 3 - full-width tray] */
row3_height       = 64;
row3_pocket_depth = 10;

/* [Magnet recesses (4 corners)] */
magnet_diameter = 5;
magnet_depth    = 3;
magnet_inset    = 5;         // hole centre offset from each corner
...
assert(y3_bot   > (ruler_enabled ? major_tick_len + ruler_number_size + 4 : 2),
       "Rows do not fit vertically - reduce row heights or gaps");
assert(max(row1_pocket_depth, row2_pocket_depth, row3_pocket_depth) < board_thickness,
...
module row3_cut() {
    pocket(edge_margin, y3_bot, avail_w, row3_height, row3_pocket_depth);
}
...
module ruler_cuts() {
    ...
    for (i = [0 : n_ticks]) {
        ...
        // tick, hanging downwards from the tray edge
        translate([xx - tick_width/2, y3_bot - tlen,
                   board_thickness - ruler_etch_depth])
            cube([tick_width, tlen, ruler_etch_depth + 0.02]);

        // number (in cm) under the major ticks
        if (major)
            etch_text(xx, y3_bot - major_tick_len - 1.5 - ruler_number_size/2,
